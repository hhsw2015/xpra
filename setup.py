...
def install_dev_env_command() -> list[str]:
    ...
    if is_Arch():
        flag_to_pkgs = {
            "modules": (
                # Keep this list focused on packages that are not guaranteed to exist
                # in a minimal Arch environment.
                "gcc", "make", "binutils", "patch", "fakeroot", "pkgconf",
            ),
            ...
            "tests": (
                ...
                "desktop-file-utils", "xclip",
            ),
            "x11": (
                "libx11", "libxcomposite", "libxdamage", "libxtst",
                "libxkbfile", "libxres",
                "xorg-server-xvfb",
            ),
            "Xdummy": ("xf86-video-dummy", ),
            "proc": ("procps-ng", ),
            "sd_listen": ("systemd", ),
            "pam": ("pam", ),
        }
        return add_flags(["pacman", "-S", "--needed"], flag_to_pkgs)
    distro = get_linux_distribution()
...
def install_repo(repo_variant="") -> None:
    ...
    if is_Arch():
        ...
        print(" use your system repositories or AUR, then run 'python3 ./setup.py dev-env' for build dependencies")
        return
...
if WIN32:
    ...
    if "install_exe" in sys.argv:
        ...
        if gtk3_ENABLED or audio_ENABLED:
            ...
            # add_dir('lib', ["gio"])
            packages.append("gi")
            add_gi_typelib("Gio-2.0", "GioWin32-2.0", "GIRepository-3.0", "Glib-2.0", "GModule-2.0", "GObject-2.0")
        if gtk3_ENABLED:
            add_gi(
                "Atk-1.0",
                "Notify-0.7",
...
if x11_ENABLED:
    ...
    ace("xpra.x11.bindings.loop", "x11,glib-2.0")
    if not WIN32:
